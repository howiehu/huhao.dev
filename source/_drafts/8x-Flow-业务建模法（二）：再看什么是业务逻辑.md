...
**业务逻辑，即业务的甲乙双方，基于业务合约的权责约定，进行履约的过程。**
...
**而所有的业务活动，其本质都是甲乙双方根据`合约（Contracts）`中的各项约定，进行`履约（Fulfillments）`的过程，这些履约过程的可追溯记录就是`凭证（Evidences）`。**
...
相比较DDD现有的分析和建模理论来说，8x Flow在业务逻辑的定义和识别方面，有着更加清晰的认知和指导，这样就能够让我更加尽早的识别问题的边界，避免陷入单纯的“发散-收敛”过程，从一定程度上防止忽略清晰的问题边界做归纳，从而造成业务与领域边界不清的情况。
