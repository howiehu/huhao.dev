...
在了解了必要的合约知识之后，我们基于大家日常接触比较多的订餐平台的业务，来讲一讲如何基于合约进行业务逻辑分析和提取。

通常来讲，**在现实中我们都应基于现有合约内容分析已有业务，或者基于合约的思想设计新的业务**，但是真正的订餐业务是非常复杂的，合约的内容也非常的多。**为了便于写文章，下面我会给出一个经过简化的订餐平台业务说明进行说明，而非采用真实的合约文本**。
...
**分析当事人的合约关系有一个比DDD现有分析方法更为显著的好处：**

**可以不需要通过完整的流程分析，而是在一开始就能够快速识别出大致准确的业务上下文边界。而DDD现有的分析方法（例如事件风暴），往往都需要经过重量级的探索过程才能得知。而且由于忽略了业务的客观存在性，并受限于所参与人员的认知能力，还不见得能准确识别出来。**

提前识别了合约的上下文有什么样的好处呢？其实这里于DDD拥有一个共同的目的，就是识别概念差异并统一语言。
